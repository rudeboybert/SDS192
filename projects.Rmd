...
1. **Baseline:** Projects that do not satisfy all "baseline" criteria can expect to get a grade of less than 8/10.
    + **This will be strictly evaluated**: The `final_project.Rmd` R Markdown file must "knit" correctly when *someone else* knits it. This is known as producing *reproducible research*.
    + Your code is clean, commented, and well-indented (See MP2 feedback video for what I mean). This is being mindful of the work you create for your collaborators, including your most important collaborator: future you. 
    + Wherever possible, all maps have appropriately labeled axes, legends, titles, etc. Such information gives the data's context.
    + All visualizations and maps are mindful of the [ink/information ratio](https://medium.com/@sudharsanasai/declutter-your-chart-with-data-ink-ratio-6f6908727842){target="_blank"}.
    + All accompanying write-ups are coherent and respect the word count limit.
    + From the report reader's perspective, there should be no "superfluous" and non-informative output (examples include instructions text). You must be empathetic to your reader, or your reader will just throw your report in the trash. 
1. **Minimally viable product:** Grade: 8/10.
    + Satisfy all "baseline" criteria.
...
* +1 for using at least 4 unique `dplyr` verbs
* +1 for having a blogpost style intro page of no more than 300 words and one visualization.
* +1 for having a "More information" tab page of no more than 800 words with between 2-3 visualizations.
* +1 for providing a clear, concise description of the data you used and its original source, which is not necessarily the site you obtained the data from.


**Advanced**

* +1 for using at least one `dplyr` `_join`
* +0–4 Awarded at the grader's discretion for submissions that are exceptionally compelling:
    + 0: assignment incomplete with little apparent effort
    + 1: graphics are relatively simple, confusing, sloppily executed, or not well thought out; analysis is muddled or uninformed
    + 2: a solid all-around effort, graphics are appropriate, labeled, and contextualized; analysis is reasonable, considered, and well-researched
    + 3: graphics are special, showing creativity, an original design, and/or exceptional attention to detail; analysis is exceptionally thorough and well thought out
    + 4: truly outstanding work; graphics are something I've never seen before, hadn't thought about, or can't immediately figure out how to do; analysis is of publishable quality
-->
    
    
## To submit

1. **Group leader**: In the Groups Google Sheet in the `#final_project` Slack team, post the URL to your RMarkdown Website published online (as long as data/work is not private or sensitive in nature).
1. **Group leader**: The RStudio Project folder necessary to build your RMarkdown Website. There will be only one submission per group. The submission method will either be Moodle or GitHub Classroom (TBD).
1. **Individually**: Fill out the peer evaluation [Google Form](https://docs.google.com/forms/d/e/1FAIpQLSfRfVN58QWyv2LRB865eA-JdCIWm-AUUx959cdGBgFo0Wk5MA/viewform){target="_blank"}
1. **Individually**: Fill out the "192 exit survey" Google Form where I'll ask you questions about your experience in 192. To be posted later.
