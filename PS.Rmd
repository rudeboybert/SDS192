...
* Download the following file <a href="static/PS/PS10.zip" download>`PS10.zip`</a>
* If you use:
    + RStudio Server: Upload the `PS10.zip` file to RStudio Server. 
    + RStudio Desktop: Move the `PS10.zip` file to where all your 192 work is and double-click it to unzip the file.
* A new folder called `PS10` should appear. Inside this folder, double-click the `PS10.Rproj` RStudio Project file to ensure you are in "PS10 mode". This will set the "working directory" of R so that R knows where to find all necessary files.
* **DO NOT IGNORE THIS**:
    + Whenever you are working on PS10, be sure to be in "PS10 RStudio Project" mode. If you do not understand what this means, ask the person sitting next to you. 
    + When you are not working on PS10, be sure to close the RStudio Project.
* Submission: Submit only the `PS10.Rmd` file on [Moodle](https://moodle.smith.edu/course/view.php?id=29919){target="_blank"}. You can assume that I will have access to the same `WA_counties/` shapefiles directory/folder.


## 3. R Markdown solutions & (imperfect) rubric {#PS10_R_solutions}
...
* R Markdown component: 2 pts:
    + 1: Does the interactive map correctly show when you knit the `.Rmd` file?
    + 1: Does the static map of Western Washington show when you knit the `.Rmd` file?
