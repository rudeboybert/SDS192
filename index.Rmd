...
* Bring Julia and Eloana prize for SDS293.
* No office hours today
* Show how to add/mute Slack channel
* How to run all code in R Markdown
* From last readings, while tibble() and not data.frame()?

1. Slack profile picture
1. If you want to get fancywith color palettes, use the [`viridis` package](https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html){target="_blank"}
...
* Mini-project 1 at end of lecture
...
* In particular in Section 2.9.3, open the `ggplot2` cheatsheet in RStudio and go over the different `geom` types
* Experiment with palettes in [colorbrewer2.org](http://colorbrewer2.org/){target="_blank"}:
    1. Copy and paste the code below into your classnotes
    1. Change both the color of the scatterplot points and the fill of the bars in the barplot by changing the `palette` argument in `scale_color_brewer()` and `scale_fill_brewer()` from `"Set1"` to something else
    1. Play around with different color palettes
...
# 1. Recreate plot from PS02, but change default "color" of points:
...
# 1. Recreate Figure 2.26, but change default "fill" color of bars:
